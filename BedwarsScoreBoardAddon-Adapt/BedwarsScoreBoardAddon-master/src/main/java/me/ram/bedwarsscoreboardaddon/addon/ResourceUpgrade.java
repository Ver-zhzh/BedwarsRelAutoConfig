				...
				arena.addGameTask(new BukkitRunnable() {
					...
					public void run() {
						...
						if (i <= 0) {
							...
							GameConfig gameConfig = ConfigHelper.getConfig(game);
							boolean drop = true;
							if (gameConfig.resourcelimit_enabled) {
								for (String[] rl : gameConfig.resourcelimit_limit) {
			...
			arena.addGameTask(new BukkitRunnable() {
				...
				public void run() {
					...
					if (game.getTimeLeft() <= gametime) {
						...
						GameConfig gameConfig = ConfigHelper.getConfig(game);
						PlaySound.playSound(game, gameConfig.play_sound_sound_upgrade);
						cancel();
					}
				}
