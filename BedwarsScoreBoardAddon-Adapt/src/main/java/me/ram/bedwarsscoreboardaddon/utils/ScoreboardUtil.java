...
public class ScoreboardUtil {
	...
	private static void sendShowHealthPacket(Player player, Game game) {
		ProtocolManager man = ProtocolLibrary.getProtocolManager();
		Arena arena = Main.getInstance().getArenaManager().getArena(game.getName());
		boolean tabHealth = arena != null ? arena.getArenaConfig().getBoolean("tab_health", Config.tab_health)
				: Config.tab_health;
		boolean tagHealth = arena != null ? arena.getArenaConfig().getBoolean("tag_health", Config.tag_health)
				: Config.tag_health;
		if (tabHealth) {
			try {
	...
	private static void sendHealthValuePacket(Player player, Player target, int value, Game game) {
		ProtocolManager man = ProtocolLibrary.getProtocolManager();
		Arena arena = Main.getInstance().getArenaManager().getArena(game.getName());
		boolean tabHealth = arena != null ? arena.getArenaConfig().getBoolean("tab_health", Config.tab_health)
				: Config.tab_health;
	...
	public static void setGameScoreboard(Player player, String title, List<String> lines, Game game) {
		...
		try {
			...
			if ((player.getScoreboard() == null || !player.getScoreboard().equals(scoreboard)) && !exist) {
				sendShowHealthPacket(player, game);
			}
			scoreboard.getObjective(DisplaySlot.SIDEBAR).setDisplayName(elements[0]);
			for (int i = 1; i < elements.length; ++i) {
				if (elements[i] != null
			...
			for (Player pl : game.getPlayers()) {
				...
				if (map.getOrDefault(pl, 0) != health) {
					sendHealthValuePacket(player, pl, health, game);
					map.put(pl, health);
				}
			}
			...
			for (Team team : game.getTeams().values()) {
				...
				if (!Config.playertag_prefix.equals("")) {
					score_team.setPrefix(Config.playertag_prefix.replace("{color}", team.getChatColor() + "")
				...
				if (!Config.playertag_suffix.equals("")) {
					score_team.setSuffix(Config.playertag_suffix.replace("{color}", team.getChatColor() + "")
							.replace("{color_initials}", team.getChatColor().name().substring(0, 1))
							.replace("{color_name}", upperInitials(team.getChatColor().name()))
							.replace("{team_initials}", team.getName().substring(0, 1))
							.replace("{team}", team.getName()));
				...
				for (Player pl : team.getPlayers()) {
					if (!score_team.hasPlayer((OfflinePlayer) pl)) {
						if (!players.contains(pl.getUniqueId())
								|| (player_team != null && player_team.getPlayers().contains(pl))) {
