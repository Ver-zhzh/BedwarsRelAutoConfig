...
public class Trap implements Upgrade {
	...
	public void runUpgrade() {
		...
		for (Player player : game.getPlayers()) {
			...
			if (team != game.getPlayerTeam(player) && team.getTargetFeetBlock()
					.distanceSquared(player.getLocation()) <= Math.pow(Config.teamshop_upgrade_trap_trigger_range, 2)) {
				...
				if (!Config.teamshop_upgrade_trap_trigger_title.equals("")
					...
					for (Player teamplayers : team.getPlayers()) {
						Utils.sendTitle(teamplayers, 5, 80, 5, Config.teamshop_upgrade_trap_trigger_title,
