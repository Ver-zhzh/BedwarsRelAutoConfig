...
public class GameChest {
	...
	public void onInteract(PlayerInteractEvent e) {
		...
		if (!arenaConfig.getBoolean("game_chest.enabled", Config.game_chest_enabled)) {
			return;
		}
		...
		if (block.getType().equals(Material.ENDER_CHEST)) {
			...
			for (Team t : game.getTeams().values()) {
				if (!teamChests.containsValue(block) && !teamChests.containsKey(t)
						&& t.getSpawnLocation().distanceSquared(location) <= Math
								.pow(arenaConfig.getInt("game_chest.range", Config.game_chest_range), 2)) {
					((Chest) block.getState()).getInventory().clear();
			...
			if (!BedwarsUtil.isDieOut(game, t)) {
				player.sendMessage(arenaConfig.getString("game_chest.message", Config.game_chest_message)
