...
public class InvisibilityPlayer {
	...
	public void hidePlayer(Player player) {
		...
		BukkitTask task = Bukkit.getScheduler().runTaskTimer(Main.getInstance(), () -> {
			if (arenaConfig.getBoolean("invisibility_player.footstep", Config.invisibility_player_footstep)) {
				arena.addGameTask(new BukkitRunnable() {
					...
					public void run() {
						if (player.isOnline() && (loc.getX() != player.getLocation().getX()
							...
							player.getWorld()
									.playEffect(player.getLocation().clone().add((Math.random() - Math.random()) * 0.5,
											0.05, (Math.random() - Math.random()) * 0.5), Effect.FOOTSTEP, 0);
						}
					}
		...
		arena.addGameTask(new BukkitRunnable() {
			...
			public void run() {
				if (!player.isOnline() || !player.hasPotionEffect(PotionEffectType.INVISIBILITY)
					...
					if (player.isOnline()) {
						showArmor(player);
						if (arenaConfig.getBoolean("invisibility_player.hide_particles",
