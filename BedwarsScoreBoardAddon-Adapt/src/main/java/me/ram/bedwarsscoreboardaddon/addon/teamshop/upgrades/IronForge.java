...
public class IronForge implements Upgrade, Listener {
		...
		for (Player player : players) {
			Bukkit.getScheduler().runTaskLater(Main.getInstance(), () -> {
				if (game.getState().equals(GameState.RUNNING)) {
					drop_items.put(dropItem(loc, itemStack, 0, true, player),
							new ItemData(player, System.currentTimeMillis()));
				}
			}, rand.nextInt(11));
		}
	}
	...
	private List<Player> getNearbyPlayers(Location location, ItemStack item) {
		...
		for (Entity entity : location.getWorld().getNearbyEntities(location, Config.spread_resource_range, 0.5,
				Config.spread_resource_range)) {
	...
	private void loadLocations() {
		if (Config.game_team_spawner.containsKey(game.getName())
				&& Config.game_team_spawner.get(game.getName()).containsKey(team.getName())) {
