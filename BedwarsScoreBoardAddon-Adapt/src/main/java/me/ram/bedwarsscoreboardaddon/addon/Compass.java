...
public class Compass implements Listener {
	...
	// private ItemStack compassItem; // Removed to support per-game config
	...
	public void onInteract(PlayerInteractEvent e) {
		...
		ItemStack compassItem = getItemItem(arenaConfig);
	...
	public void onOpen(InventoryOpenEvent e) {
		// This check is tricky because we don't know the game context easily without
		// player.
		// But usually InventoryOpenEvent has a player.
		if (e.getPlayer() instanceof Player) {
			...
			if (game != null) {
				...
				if (e.getInventory().getTitle()
						.equals(arenaConfig.getString("compass.gui_title", Config.compass_gui_title))) {
	...
	public void onClick(InventoryClickEvent e) {
		...
		String guiTitle = arenaConfig.getString("compass.gui_title", Config.compass_gui_title);
		...
		} else if (inventory.getTitle().equals(guiTitle + "§a§i")) {
			e.setCancelled(true);
			int slot = e.getRawSlot();
			if (slot == 31) {
				openMainMenu(player);
		...
		} else if (inventory.getTitle().equals(guiTitle + "§a")) {
			e.setCancelled(true);
			int slot = e.getRawSlot();
			if (slot == 31) {
				openMainMenu(player);
		...
		} else if (inventory.getTitle().equals(guiTitle + "§c")) {
			e.setCancelled(true);
			int slot = e.getRawSlot();
			if (slot == 31) {
				openMainMenu(player);
		...
		} else if (inventory.getTitle().equals(guiTitle + "§n")) {
			e.setCancelled(true);
			int slot = e.getRawSlot();
			if (slot == 31) {
				openMainMenu(player);
		...
		} else if (inventory.getTitle().equals(guiTitle + "§h")) {
			e.setCancelled(true);
			int slot = e.getRawSlot();
			if (slot == 31) {
				openMainMenu(player);
	...
	private void openMainMenu(Player player) {
		...
		Inventory inventory = Bukkit.createInventory(null, 36,
				arenaConfig.getString("compass.gui_title", Config.compass_gui_title));
	...
	private void openSelectTeamMenu(Player player, String gui) {
		...
		ArenaConfig arenaConfig = arena.getArenaConfig();
		Inventory inventory = Bukkit.createInventory(null, 36,
				arenaConfig.getString("compass.gui_title", Config.compass_gui_title) + gui);
		int i = 10;
	...
	private static ItemStack getItemItem(ArenaConfig arenaConfig) {
		ItemStack itemStack = new ItemStack(Material.COMPASS);
		ItemMeta itemMeta = itemStack.getItemMeta();
		itemMeta.setDisplayName(arenaConfig.getString("compass.item_name", Config.compass_item_name));
		// Handle lore list
		java.util.List<String> lore = arenaConfig.getStringList("compass.item_lore");
		if (lore == null || lore.isEmpty())
			lore = Config.compass_item_lore;
	...
	public static void giveCompass(Player player) {
		...
		Arena arena = Main.getInstance().getArenaManager().getArena(game.getName());
		ArenaConfig arenaConfig = arena.getArenaConfig();
		player.getInventory().setItem(8, getItemItem(arenaConfig));
	}
}
