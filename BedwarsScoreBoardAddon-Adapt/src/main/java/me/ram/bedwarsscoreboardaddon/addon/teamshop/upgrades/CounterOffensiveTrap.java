...
public class CounterOffensiveTrap implements Upgrade {
		...
		for (Player player : game.getPlayers()) {
			...
			if (team != game.getPlayerTeam(player)
					&& team.getTargetFeetBlock().distanceSquared(player.getLocation()) <= Math
							.pow(Config.teamshop_upgrade_counter_offensive_trap_trigger_range, 2)) {
	...
	private void addEffect() {
		for (Player player : team.getPlayers()) {
			if (!player.getGameMode().equals(GameMode.SPECTATOR) && !BedwarsUtil.isSpectator(game, player)
					&& team.getTargetFeetBlock().distance(
							player.getLocation()) <= Config.teamshop_upgrade_counter_offensive_trap_effect_range) {
