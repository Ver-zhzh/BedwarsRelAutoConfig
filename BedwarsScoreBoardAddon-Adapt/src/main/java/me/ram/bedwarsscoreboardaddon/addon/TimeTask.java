...
public class TimeTask {
	...
	public TimeTask(Arena arena) {
		...
		for (String cmd : arenaConfig.getStringList("timecommand.startcommand", Config.timecommand_startcommand)) {
			if (cmd.equals("")) {
				continue;
			}
		...
		if (arenaConfig.getConfigurationSection("timecommand") != null) {
			for (String cmds : arenaConfig.getConfigurationSection("timecommand").getKeys(false)) {
				if (cmds.equals("startcommand"))
					continue;
				arena.addGameTask(new BukkitRunnable() {
					int gametime = arenaConfig.getInt("timecommand." + cmds + ".gametime", 0);
					List<String> cmdlist = arenaConfig.getStringList("timecommand." + cmds + ".command");
