...
public class ScoreBoard {
	...
	public ScoreBoard(Arena arena) {
		...
		ConfigurationSection timerSection = arena.getArenaConfig().getConfig().getConfigurationSection("timer");
		if (timerSection != null) {
			for (String id : timerSection.getKeys(false)) {
				arena.addGameTask(new BukkitRunnable() {
					int i = timerSection.getInt(id);
		...
		arena.addGameTask(new BukkitRunnable() {
			int i = arena.getArenaConfig().getInt("scoreboard.interval", 20);
			...
			public void run() {
				...
				if (i <= 0) {
					updateScoreboard();
					i = arena.getArenaConfig().getInt("scoreboard.interval", 20);
				}
			}
	...
	public void updateScoreboard() {
		...
		ConfigurationSection planSection = arena.getArenaConfig().getConfig().getConfigurationSection("planinfo");
		if (planSection != null) {
			for (String plan : planSection.getKeys(false)) {
				if (game.getTimeLeft() <= planSection.getInt(plan + ".start_time")
						&& game.getTimeLeft() > planSection.getInt(plan + ".end_time")) {
					ConfigurationSection plans = planSection.getConfigurationSection(plan + ".plans");
					if (plans != null) {
						for (String key : plans.getKeys(false)) {
							plan_infos.put(key, plans.getString(key));
						}
					}
				}
			}
		...
		int wither = game.getTimeLeft() - arena.getArenaConfig().getInt("witherbow.gametime", 0);
		...
		if (wither <= 0) {
			bowtime = arena.getArenaConfig().getString("witherbow.already_starte", "");
		}
		String score_title = "";
		List<String> scoreboard_titles = arena.getArenaConfig().getConfig().getStringList("scoreboard.title");
		if (title_index >= scoreboard_titles.size()) {
			title_index = 0;
		}
		score_title = scoreboard_titles.size() < 1 ? "BedWars"
				: scoreboard_titles.get(title_index).replace("{game}", game.getName()).replace("{time}",
						getFormattedTimeLeft(game.getTimeLeft()));
		title_index++;
		String teams = game.getTeams().size() + "";
		List<String> scoreboard_lines;
		ConfigurationSection linesSection = arena.getArenaConfig().getConfig()
				.getConfigurationSection("scoreboard.lines");
		if (linesSection != null && linesSection.contains(teams)) {
			scoreboard_lines = linesSection.getStringList(teams);
		} else if (linesSection != null && linesSection.contains("default")) {
			scoreboard_lines = linesSection.getStringList("default");
		} else {
		...
		for (Player player : game.getPlayers()) {
			...
			lines.clear();
			String player_total_kills = arena.getPlayerGameStorage().getPlayerTotalKills()
					.getOrDefault(player.getName(), 0) + "";
			String player_kills = arena.getPlayerGameStorage().getPlayerKills().getOrDefault(player.getName(), 0) + "";
			String player_final_kills = arena.getPlayerGameStorage().getPlayerFinalKills()
					.getOrDefault(player.getName(), 0) + "";
			...
			for (String ls : scoreboard_lines) {
				if (ls.contains("{team_status}")) {
					for (Team t : game.getTeams().values()) {
						...
						if (game.getPlayerTeam(player) != null) {
							if (game.getPlayerTeam(player) == t) {
								you = arena.getArenaConfig().getString("scoreboard.you", "");
							} else {
								you = "";
							}
						}
					...
					String date = new SimpleDateFormat(arena.getArenaConfig().getString("date_format", "yyyy/MM/dd"))
							.format(new Date());
					String add_line = ls;
					...
					add_line = add_line.replace("{death_mode}", arena.getDeathMode().getDeathmodeTime())
							.replace("{remain_teams}", remain_teams + "").replace("{alive_teams}", alive_teams + "")
							.replace("{alive_players}", alive_players + "")
							.replace("{teams}", game.getTeams().size() + "").replace("{color}", player_team_color)
							.replace("{team_peoples}", player_team_players).replace("{player_name}", player.getName())
							.replace("{team}", player_team_name).replace("{beds}", player_bes)
							.replace("{dies}", player_dis).replace("{totalkills}", player_total_kills)
					...
					for (String key : arena.getHealthLevel().getLevelTime().keySet()) {
						add_line = add_line.replace("{sethealthtime_" + key + "}",
								arena.getHealthLevel().getLevelTime().get(key));
					}
					for (String key : arena.getResourceUpgrade().getUpgTime().keySet()) {
						add_line = add_line.replace("{resource_upgrade_" + key + "}",
								arena.getResourceUpgrade().getUpgTime().get(key));
					}
					for (String key : placeholderManager.getGamePlaceholder().keySet()) {
						add_line = add_line.replace(key,
								placeholderManager.getGamePlaceholder().get(key).onGamePlaceholderRequest(game));
					}
					for (Team t : game.getTeams().values()) {
						if (add_line.contains("{team_" + t.getName() + "_status}")) {
							...
							if (game.getPlayerTeam(player) == null) {
								...
								stf = stf.replace("{you}", arena.getArenaConfig().getString("scoreboard.you", ""));
							} else {
								stf = stf.replace("{you}", "");
							}
						...
						if (add_line.contains("{team_" + t.getName() + "_bed_status}")) {
							add_line = add_line.replace("{team_" + t.getName() + "_bed_status}",
									getTeamBedStatus(game, t));
						}
						if (add_line.contains("{team_" + t.getName() + "_peoples}")) {
							add_line = add_line.replace("{team_" + t.getName() + "_peoples}",
									t.getPlayers().size() + "");
						}
					}
					if (player_team == null
							|| !placeholderManager.getTeamPlaceholders().containsKey(player_team.getName())) {
						...
						for (String identifier : placeholderManager.getTeamPlaceholder(player_team.getName())
								.keySet()) {
							add_line = add_line.replace(identifier,
									placeholderManager.getTeamPlaceholder(player_team.getName()).get(identifier)
											.onTeamPlaceholderRequest(player_team));
						}
					}
					if (placeholderManager.getPlayerPlaceholders().containsKey(player.getName())) {
						for (String identifier : placeholderManager.getPlayerPlaceholder(player.getName()).keySet()) {
							add_line = add_line.replace(identifier,
									placeholderManager.getPlayerPlaceholder(player.getName()).get(identifier)
											.onPlayerPlaceholderRequest(game, player));
						}
					} else {
	...
	private String getTeamBedStatus(Game game, Team team) {
		return team.isDead(game) ? arena.getArenaConfig().getString("scoreboard.team_bed_status.bed_destroyed", "")
				: arena.getArenaConfig().getString("scoreboard.team_bed_status.bed_alive", "");
	}

	private String getTeamStatusFormat(Game game, Team team) {
		String alive = arena.getArenaConfig().getString("scoreboard.team_status_format.bed_alive", "");
		String destroyed = arena.getArenaConfig().getString("scoreboard.team_status_format.bed_destroyed", "");
		String status = team.isDead(game) ? destroyed : alive;
		if (team.isDead(game) && team.getPlayers().size() <= 0) {
			status = arena.getArenaConfig().getString("scoreboard.team_status_format.team_dead", "");
		}
		return status.replace("{bed_status}", getTeamBedStatus(game, team)).replace("{color}", team.getChatColor() + "")
				...
				.replace("{team_initials}", team.getName().substring(0, 1)).replace("{team}", team.getName());
	}
