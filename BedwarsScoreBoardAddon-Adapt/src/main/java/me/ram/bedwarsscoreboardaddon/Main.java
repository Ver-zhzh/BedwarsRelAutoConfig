...
public class Main extends JavaPlugin {
	...
	public void onEnable() {
		if (!getDescription().getName().equals("BedwarsScoreBoardAddon")
				...
				|| !getDescription().getAuthors().contains("Ram")) {
			try {
		...
		new BukkitRunnable() {
			...
			public void run() {
				if (Bukkit.getPluginManager().getPlugin("BedwarsRel") == null
					...
					printMessage("§7 ");
					printMessage("§f  " + getLocaleConfig().getLanguage("website")
							+ ": §ehttps://github.com/TheRamU/BedwarsScoreBoardAddon");
					printMessage("§7 ");
					printMessage("§f===========================================================");
					init();
				}
