...
public class GameListener implements Listener {
	...
	public void onOpenTeamSelection(BedwarsOpenTeamSelectionEvent e) {
		Arena arena = Main.getInstance().getArenaManager().getArenas().get(e.getGame().getName());
		if (arena == null) {
			return;
		}
		if (!arena.getArenaConfig().getBoolean("select_team.enabled", Config.select_team_enabled)) {
			return;
		}
		e.setCancelled(true);
