...
public class Arena {
		...
		if (e.getGame().getName().equals(this.game.getName())) {
			...
			if (Config.overstats_enabled && e.getWinner() != null) {
				...
				for (Player player : winner.getPlayers()) {
					win_team_player_list += win_team_player_list.length() > 0 ? ", " + player.getName()
							: player.getName();
				}
	...
	public void onItemMerge(ItemMergeEvent e) {
		if (!arenaConfig.getBoolean("item_merge", Config.item_merge)
				&& game.getRegion().isInRegion(e.getEntity().getLocation())) {
			e.setCancelled(true);
		}
	}
	...
	public void onPlayerLeave(Player player) {
		...
		if (arenaConfig.getBoolean("rejoin.enabled", Config.rejoin_enabled)) {
