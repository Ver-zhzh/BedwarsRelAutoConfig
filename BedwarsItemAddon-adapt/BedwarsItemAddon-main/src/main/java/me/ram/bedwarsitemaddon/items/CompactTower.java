...
public class CompactTower implements Listener {
    ...
    public void onBlockPlace(BlockPlaceEvent e) {
        ...
        if (!Config.getBoolean(gameName, "compact_tower.enabled", Config.items_compact_tower_enabled)) {
            return;
        }
        ...
        String itemMaterial = Config.getString(gameName, "compact_tower.item", Config.items_compact_tower_item);
        if (e.getItemInHand().getType() != new ItemStack(Material.valueOf(itemMaterial)).getType()) {
            return;
        }
        e.setCancelled(true);
        double cooldownTime = Config.getDouble(gameName, "compact_tower.cooldown", Config.items_compact_tower_cooldown);
        if ((System.currentTimeMillis()
                - cooldown.getOrDefault(player, (long) 0)) <= cooldownTime * 1000) {
            player.sendMessage(
                    Config.message_cooling.replace("{time}",
                            String.format("%.1f", (((cooldownTime * 1000
                                    - System.currentTimeMillis() + cooldown.getOrDefault(player, (long) 0)) / 1000)))
                                    + ""));
            return;
        }
        ...
        if (Bukkit.getPluginManager().isPluginEnabled("BedwarsScoreBoardAddon")) {
            if (me.ram.bedwarsscoreboardaddon.config.Config.spawn_no_build_spawn_enabled) {
                for (Team team : game.getTeams().values()) {
                    if (team.getSpawnLocation().distanceSquared(block.getLocation().clone().add(0.5, 0, 0.5)) <= Math
                            .pow(me.ram.bedwarsscoreboardaddon.config.Config.spawn_no_build_spawn_range, 2)) {
                        return false;
                    }
                }
            }
            if (me.ram.bedwarsscoreboardaddon.config.Config.spawn_no_build_resource_enabled) {
                for (ResourceSpawner spawner : game.getResourceSpawners()) {
                    if (spawner.getLocation().distanceSquared(block.getLocation().clone().add(0.5, 0, 0.5)) <= Math
                            .pow(me.ram.bedwarsscoreboardaddon.config.Config.spawn_no_build_resource_range, 2)) {
                        return false;
                    }
                }
                if (me.ram.bedwarsscoreboardaddon.config.Config.game_team_spawner.containsKey(game.getName())) {
                    for (List<Location> locs : me.ram.bedwarsscoreboardaddon.config.Config.game_team_spawner
                            .get(game.getName()).values()) {
                        for (Location loc : locs) {
                            if (loc.distanceSquared(block.getLocation().clone().add(0.5, 0, 0.5)) <= Math.pow(
                                    me.ram.bedwarsscoreboardaddon.config.Config.spawn_no_build_resource_range, 2)) {
                                return false;
                            }
                        }
                    }
