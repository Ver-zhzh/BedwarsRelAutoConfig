...
public class ExplosionProof implements Listener {
    ...
    public void onExplode(EntityExplodeEvent e) {
        ...
        if (!Config.getBoolean(gameName, "explosion_proof.enabled", Config.items_explosion_proof_enabled)) {
            return;
        }
        ...
        for (Block block : e.blockList()) {
            ...
            boolean isadd = true;
            Iterator<Block> itr = new BlockIterator(location.getWorld(), location.toVector(),
                    loc.clone().subtract(location).toVector(), 0, (int) Math.ceil(loc.distance(location)));
            while (itr.hasNext()) {
                Block b = itr.next();
