...
public class BridgeEgg implements Listener {
    ...
    public void onInteract(PlayerInteractEvent e) {
        ...
        if (!Config.getBoolean(gameName, "bridge_egg.enabled", Config.items_bridge_egg_enabled)) {
            return;
        }
        ...
        if ((e.getAction().equals(Action.RIGHT_CLICK_AIR) || e.getAction().equals(Action.RIGHT_CLICK_BLOCK))
                && e.getItem().getType() == new ItemStack(Material.EGG).getType()) {
            double cooldownTime = Config.getDouble(gameName, "bridge_egg.cooldown", Config.items_bridge_egg_cooldown);
            if ((System.currentTimeMillis() - cooldown.getOrDefault(player, (long) 0)) <= cooldownTime * 1000) {
                e.setCancelled(true);
                player.sendMessage(Config.message_cooling.replace("{time}", String.format("%.1f",
                        (((cooldownTime * 1000 - System.currentTimeMillis() + cooldown.getOrDefault(player, (long) 0))
                                / 1000)))
                        + ""));
            } else {
                ItemStack stack = e.getItem();
                BedwarsUseItemEvent bedwarsUseItemEvent = new BedwarsUseItemEvent(game, player, EnumItem.BRIDGE_EGG,
                        stack);
    ...
    public void setblock(Game game, Egg egg, Player player) {
        ...
        final int maxBlock = Config.getInt(gameName, "bridge_egg.maxblock", Config.items_bridge_egg_maxblock);
        new BukkitRunnable() {
            int i = 0;
                    ...
                    new BukkitRunnable() {
                        ...
                        public void run() {
                            ...
                            for (Location loc : blocklocation) {
                                ...
                                if (block.getType() == new ItemStack(Material.AIR).getType()
                                        && !block.equals(player.getLocation().getBlock())
                                        && !block.equals(player.getLocation().clone().add(0, 1, 0).getBlock())
                                        && game.getRegion().isInRegion(loc) && i < maxBlock) {
                                    loc.getBlock().setType(Material.WOOL);
                                    loc.getBlock()
                                            .setData(game.getPlayerTeam(player).getColor().getDyeColor().getWoolData());
                                    i++;
