...
public class Config {
    ...
    // ============ Per-Game Configuration API ============
    // 存储每个游戏的自定义 items 配置
    private static Map<String, FileConfiguration> gameItemsConfigs = new HashMap<>();
    // 默认的 items 配置（从 items.yml 加载）
    private static FileConfiguration defaultItemsConfig;
    ...
    public static boolean items_trampoline_no_fall;
    ...
    public static void loadConfig() {
        ...
        // 保存默认配置用于 per-game API
        defaultItemsConfig = items_config;
        ...
        items_trampoline_no_fall = items_config.getBoolean("trampoline.no_fall");
     ...
     * @param config   自定义配置
     */
    public static void setGameItemsConfig(String gameName, FileConfiguration config) {
        if (gameName != null && config != null) {
            gameItemsConfigs.put(gameName, config);
        }
    }

    /**
     * 获取游戏的 items 配置（优先返回自定义，否则返回默认）
     * 
     * @param gameName 游戏名称
     * @return 配置对象
     */
    public static FileConfiguration getGameItemsConfig(String gameName) {
        if (gameName != null && gameItemsConfigs.containsKey(gameName)) {
            // System.out.println("[BedwarsItemAddon] DEBUG: Found custom config for game: "
            // + gameName);
            return gameItemsConfigs.get(gameName);
        }
        // System.out.println("[BedwarsItemAddon] DEBUG: No custom config for game: " +
        // gameName + ", using default.");
        return defaultItemsConfig;
    }
     ...
     * @param gameName 游戏名称
     */
    public static void removeGameItemsConfig(String gameName) {
        if (gameName != null && gameItemsConfigs.containsKey(gameName)) {
            gameItemsConfigs.remove(gameName);
        }
    }
     ...
     * @return 是否有自定义配置
     */
    public static boolean hasGameItemsConfig(String gameName) {
        return gameName != null && gameItemsConfigs.containsKey(gameName);
    }
     ...
     * @return 默认配置
     */
    public static FileConfiguration getDefaultItemsConfig() {
        return defaultItemsConfig;
    }

    // ============ Per-Game Configuration Getters ============

    /**
     * 从 per-game 配置获取 boolean 值
     */
    public static boolean getBoolean(String gameName, String key, boolean defaultValue) {
        FileConfiguration config = getGameItemsConfig(gameName);
        boolean val = defaultValue;
        if (config != null && config.contains(key)) {
            val = config.getBoolean(key, defaultValue);
        }
        return val;
    }

    /**
     * 从 per-game 配置获取 int 值
     */
    public static int getInt(String gameName, String key, int defaultValue) {
        FileConfiguration config = getGameItemsConfig(gameName);
        int val = defaultValue;
        if (config != null && config.contains(key)) {
            val = config.getInt(key, defaultValue);
        }
        return val;
    }

    /**
     * 从 per-game 配置获取 double 值
     */
    public static double getDouble(String gameName, String key, double defaultValue) {
        FileConfiguration config = getGameItemsConfig(gameName);
        double val = defaultValue;
        if (config != null && config.contains(key)) {
            val = config.getDouble(key, defaultValue);
        }
        return val;
    }

    /**
     * 从 per-game 配置获取 String 值
     */
    public static String getString(String gameName, String key, String defaultValue) {
        FileConfiguration config = getGameItemsConfig(gameName);
        String val = defaultValue;
        if (config != null && config.contains(key)) {
            val = config.getString(key, defaultValue);
        }
        return val;
    }
}
