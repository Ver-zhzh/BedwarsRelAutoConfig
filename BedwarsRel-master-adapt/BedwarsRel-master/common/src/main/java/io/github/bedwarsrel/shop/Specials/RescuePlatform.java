...
public class RescuePlatform extends SpecialItem {
  ...
  public void create(Player player, Game game) {
    ...
    // Use per-game configuration
    int breakTime = game.getGameConfig()
        .getInt("specials.rescue-platform.break-time", 10);
    int waitTime = game.getGameConfig()
        .getInt("specials.rescue-platform.using-wait-time", 20);
    boolean canBreak = game.getGameConfig()
        .getBoolean("specials.rescue-platform.can-break", false);
    Material configMaterial = Utils.getMaterialByConfig(game, "specials.rescue-platform.block", Material.STAINED_GLASS);
    ...
    if (player.getLocation().getBlock().getRelative(BlockFace.DOWN).getType() != Material.AIR) {
      player.sendMessage(
          ChatWriter.pluginMessage(game, ChatColor.RED + BedwarsRel._l(player, "errors.notinair")));
      return;
    }
  ...
  public Material getItemMaterial() {
    return Utils.getMaterialByConfig(this.game, "specials.rescue-platform.item", Material.BLAZE_ROD);
  }
