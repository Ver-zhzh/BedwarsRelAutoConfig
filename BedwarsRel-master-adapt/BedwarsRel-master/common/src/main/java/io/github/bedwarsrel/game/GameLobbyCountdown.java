...
public class GameLobbyCountdown extends BukkitRunnable {
  ...
  public GameLobbyCountdown(Game game) {
    this.game = game;
    this.counter = this.game.getGameConfig().getInt("lobbytime");
    this.lobbytime = this.counter;
    this.lobbytimeWhenFull = this.game.getGameConfig().getInt("lobbytime-full");
  }
  ...
  public void run() {
    ...
    if (this.counter > this.lobbytimeWhenFull
      ...
      for (Player aPlayer : players) {
        if (aPlayer.isOnline()) {
          aPlayer.sendMessage(ChatWriter.pluginMessage(this.game, ChatColor.YELLOW
              + BedwarsRel
    ...
    if (!this.game.isStartable()) {
      if (!this.game.hasEnoughPlayers()) {
        for (Player aPlayer : players) {
          if (aPlayer.isOnline()) {
            aPlayer.sendMessage(ChatWriter.pluginMessage(this.game,
