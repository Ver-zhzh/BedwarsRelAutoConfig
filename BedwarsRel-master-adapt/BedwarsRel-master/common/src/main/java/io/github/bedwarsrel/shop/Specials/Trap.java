...
public class Trap extends SpecialItem {
  ...
  public void activate(final Player player) {
    try {
      ConfigurationSection section = BedwarsRel.getInstance().getConfig().getConfigurationSection("specials.trap");
      ...
      for (Player aPlayer : this.team.getPlayers()) {
        if (aPlayer.isOnline()) {
          aPlayer.sendMessage(
              ChatWriter.pluginMessage(this.game, BedwarsRel._l(aPlayer, "ingame.specials.trap.trapped")));
        }
      }
      if (this.playSound) {
