...
public class EntityListener extends BaseListener {
  ...
  public void onEntityDamage(EntityDamageEvent ede) {
    ...
    Game game = BedwarsRel.getInstance().getGameManager().getGameByLocation(ede.getEntity().getLocation());
    if (game == null) {
      return;
    }
  ...
  public void onExplodeDestroy(EntityExplodeEvent eev) {
    ...
    Game game = BedwarsRel.getInstance().getGameManager().getGameByLocation(eev.getEntity().getLocation());
    ...
    boolean tntDestroyEnabled = game.getGameConfig().getBoolean("explodes.destroy-worldblocks", false);
    boolean tntDestroyBeds = game.getGameConfig()
        .getBoolean("explodes.destroy-beds", false);

    if (!game.getGameConfig().getBoolean("explodes.drop-blocks", false)) {
      eev.setYield(0F);
    }
