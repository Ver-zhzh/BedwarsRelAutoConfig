...
public final class Utils {
  ...
  /**
   * Get material from per-game config with fallback to global config.
   * 
   * @param game            The game context for per-game configuration
   * @param key             The configuration key
   * @param defaultMaterial The default material if not found
   * @return The material from config or default
   */
  @SuppressWarnings("deprecation")
  public static Material getMaterialByConfig(Game game, String key, Material defaultMaterial) {
    try {
      String cfg = game.getGameConfig().getString(key, defaultMaterial.name());
      if (Utils.isNumber(cfg)) {
        return Material.getMaterial(Integer.valueOf(cfg));
      } else {
        return Material.getMaterial(cfg.toUpperCase());
   ...
   * logical Unicode
   * code point. No bloody brain-damaged UTF-16 surrogate crap, just true logical
   * characters.
   */
