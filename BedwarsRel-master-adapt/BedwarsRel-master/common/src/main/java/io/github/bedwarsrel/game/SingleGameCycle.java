  ...
  public void onGameOver(GameOverTask task) {
    if (task.getCounter() == task.getStartCount() && task.getWinner() != null) {
      for (Player aPlayer : this.getGame().getPlayers()) {
        if (aPlayer.isOnline()) {
          aPlayer.sendMessage(
              ChatWriter.pluginMessage(this.getGame(), ChatColor.GOLD + BedwarsRel._l(aPlayer, "ingame.teamwon",
      ...
      for (Player aPlayer : this.getGame().getPlayers()) {
        if (aPlayer.isOnline()) {
          aPlayer.sendMessage(
              ChatWriter.pluginMessage(this.getGame(), ChatColor.GOLD + BedwarsRel._l(aPlayer, "ingame.draw")));
        }
      }
    }
  ...
  public boolean onPlayerJoins(Player player) {
    if (this.getGame().isFull() && !player.hasPermission("bw.vip.joinfull")) {
      if (this.getGame().getState() != GameState.RUNNING
        ...
        player.sendMessage(
            ChatWriter.pluginMessage(this.getGame(), ChatColor.RED + BedwarsRel._l(player, "lobby.gamefull")));
        return false;
      }
      ...
      if (this.getGame().getState() == GameState.WAITING) {
        ...
        if (players.size() == 0) {
          player.sendMessage(
              ChatWriter
                  .pluginMessage(this.getGame(), ChatColor.RED + BedwarsRel._l(player, "lobby.gamefullpremium")));
          return false;
        }
        ...
        kickPlayer
            .sendMessage(
                ChatWriter.pluginMessage(this.getGame(), ChatColor.RED + BedwarsRel
        ...
        if (this.getGame().getState() == GameState.RUNNING
          ...
          player.sendMessage(
              ChatWriter
                  .pluginMessage(this.getGame(), ChatColor.RED + BedwarsRel._l(player, "errors.cantjoingame")));
          return false;
        }
      }
    }
