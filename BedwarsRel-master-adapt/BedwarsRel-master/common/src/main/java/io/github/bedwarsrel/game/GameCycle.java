...
public abstract class GameCycle {
  ...
  public void onPlayerDies(Player player, Player killer) {
    ...
    if (BedwarsRel.getInstance().statisticsEnabled()) {
      ...
      boolean onlyOnBedDestroy = BedwarsRel.getInstance().getBooleanConfig("statistics.bed-destroyed-kills", false);
      ...
      if (BedwarsRel.getInstance().getBooleanConfig("rewards.enabled", false) && killer != null
        ...
        List<String> commands = this.getGame().getGameConfig()
    ...
    if (killer == null) {
      for (Player aPlayer : this.getGame().getPlayers()) {
        if (aPlayer.isOnline()) {
          aPlayer.sendMessage(
              ChatWriter.pluginMessage(this.getGame(),
    ...
    for (Player aPlayer : this.getGame().getPlayers()) {
      if (aPlayer.isOnline()) {
        aPlayer.sendMessage(
            ChatWriter.pluginMessage(this.getGame(), ChatColor.GOLD + BedwarsRel._l(aPlayer, "ingame.player.killed",
  ...
  private void runGameOver(Team winner) {
    ...
    int delay = this.getGame().getGameConfig().getInt("gameoverdelay"); // configurable
    // delay
      ...
      if (winner != null) {
        for (Player player : winner.getPlayers()) {
          ...
          if (!"".equals(title) || !"".equals(subtitle)) {
            if (BedwarsRel.getInstance().getBooleanConfig("titles.win.enabled", true)
              ...
              try {
                ...
                if (!"".equals(title)) {
                  double titleFadeIn = this.getGame().getGameConfig()
                      .getDouble("titles.win.title-fade-in", 1.5);
                  double titleStay = this.getGame().getGameConfig().getDouble("titles.win.title-stay", 5.0);
                  double titleFadeOut = this.getGame().getGameConfig().getDouble("titles.win.title-fade-out", 2.0);
                ...
                if (!"".equals(subtitle)) {
                  double subTitleFadeIn = this.getGame().getGameConfig().getDouble("titles.win.subtitle-fade-in", 1.5);
                  double subTitleStay = this.getGame().getGameConfig()
                      .getDouble("titles.win.subtitle-stay", 5.0);
                  double subTitleFadeOut = this.getGame().getGameConfig().getDouble("titles.win.subtitle-fade-out",
                      2.0);
          ...
          if (BedwarsRel.getInstance().getBooleanConfig("rewards.enabled", false)) {
            ...
            commands = (List<String>) this.getGame().getGameConfig()
    ...
    if (playTime <= this.getGame().getRecord()) {
      ...
      if (!throughBed) {
        for (Player aPlayer : this.getGame().getPlayers()) {
          if (aPlayer.isOnline()) {
            aPlayer.sendMessage(
                ChatWriter.pluginMessage(this.getGame(), BedwarsRel._l(aPlayer, "ingame.record-nobeddestroy")));
          }
        }
        return false;
      }
      ...
      for (Player aPlayer : this.getGame().getPlayers()) {
        if (aPlayer.isOnline()) {
          aPlayer.sendMessage(
              ChatWriter.pluginMessage(this.getGame(), BedwarsRel._l(aPlayer, "ingame.newrecord",
