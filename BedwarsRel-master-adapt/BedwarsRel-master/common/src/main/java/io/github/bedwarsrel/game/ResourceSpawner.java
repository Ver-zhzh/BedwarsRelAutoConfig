...
public class ResourceSpawner implements Runnable, ConfigurationSerializable {
  ...
  public ResourceSpawner(Map<String, Object> deserialize) {
    ...
    if (deserialize.containsKey("name")) {
      ...
      // Note: During deserialization, this.game is null, so we must use global
      // config.
      // Per-game config will be applied when the game starts via
      ...
      if (BedwarsRel.getInstance().getConfig().contains("resource." + name)) {
        ...
        this.interval = BedwarsRel.getInstance().getIntConfig("resource." + name + ".spawn-interval", 1000);
        this.spread = BedwarsRel.getInstance().getConfig().getDouble("resource." + name + ".spread", 1.0);
      } else {
        // Fallback to deserialize data if resource type not found in config
  ...
  public ResourceSpawner(Game game, String name, Location location) {
    this.game = game;
    this.name = name;
    this.interval = this.game.getGameConfig().getInt("resource." + name + ".spawn-interval", 1000);
    this.location = location;
    // Use per-game config if available, fallback to global
    List<Object> resourceList = (List<Object>) this.game.getGameConfig()
        .getList("resource." + name + ".item");
    for (Object resource : resourceList) {
      ItemStack itemStack = ItemStack.deserialize((Map<String, Object>) resource);
      if (itemStack != null) {
    ...
    this.spread = this.game.getGameConfig().getDouble("resource." + name + ".spread", 1.0);
  }
  ...
  public void run() {
    ...
    for (ItemStack itemStack : this.resources) {
      ...
      BedwarsResourceSpawnEvent resourceSpawnEvent = new BedwarsResourceSpawnEvent(this.game, this.location, item);
      ...
      if (this.game.getGameConfig().getBoolean("spawn-resources-in-chest", true)) {
