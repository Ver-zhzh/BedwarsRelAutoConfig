...
public class TNTSheep extends SpecialItem {
  ...
  public void run(Location startLocation) {
    ...
    if (BedwarsRel.getInstance().getCurrentVersion().startsWith("v1_8")) {
      ...
      if (usedStack.getData() instanceof SpawnEgg) {
        SpawnEgg egg = (SpawnEgg) usedStack.getData();
        if (egg.getSpawnedType() != EntityType.SHEEP) {
          return;
        }
        usedStack.setAmount(usedStack.getAmount() - 1);
        player.getInventory().setItem(player.getInventory().getHeldItemSlot(), usedStack);
      } else {
        return; // If it's not a SpawnEgg, we can't proceed with TNTSheep logic
      }
    } else {
        ...
        try {
          ...
          new BukkitRunnable() {
            ...
            public void run() {
              ...
              (long) ((BedwarsRel.getInstance().getConfig().getDouble("specials.tntsheep.fuse-time", 8.0)
                  * 20) - 5));
