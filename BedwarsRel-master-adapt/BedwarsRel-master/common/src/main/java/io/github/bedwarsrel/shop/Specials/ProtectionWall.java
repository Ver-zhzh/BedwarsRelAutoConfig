...
public class ProtectionWall extends SpecialItem {
  ...
  public void create(Player player, Game game) {
    ...
    int breakTime = game.getGameConfig().getInt("specials.protection-wall.break-time", 0);
    int waitTime = game.getGameConfig().getInt("specials.protection-wall.wait-time", 20);
    int width = game.getGameConfig().getInt("specials.protection-wall.width", 4);
    int height = game.getGameConfig().getInt("specials.protection-wall.height", 4);
    int distance = game.getGameConfig().getInt("specials.protection-wall.distance", 2);
    boolean canBreak = game.getGameConfig().getBoolean("specials.protection-wall.can-break", true);
    Material blockMaterial = Utils.getMaterialByConfig(game, "specials.protection-wall.block", Material.SANDSTONE);
    ...
    if (waitTime > 0) {
      ...
      if (!livingWalls.isEmpty()) {
        for (ProtectionWall livingWall : livingWalls) {
          ...
          if (waitLeft > 0) {
            player.sendMessage(
                ChatWriter.pluginMessage(this.game,
  ...
  public Material getItemMaterial() {
    return Utils.getMaterialByConfig(this.game, "specials.protection-wall.item", Material.BRICK);
  }
