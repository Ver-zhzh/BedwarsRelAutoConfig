...
public class ArrowBlocker extends SpecialItem {
  ...
  public void create(Player player, Game game) {
    ...
    int protectionTime = game.getGameConfig()
        .getInt("specials.arrow-blocker.protection-time", 10);
    int waitTime = game.getGameConfig()
        .getInt("specials.arrow-blocker.using-wait-time", 30);
    ...
    if (waitTime > 0) {
      ...
      if (!livingBlockers.isEmpty()) {
        for (ArrowBlocker livingBlocker : livingBlockers) {
          ...
          if (waitLeft > 0) {
            player.sendMessage(
                ChatWriter.pluginMessage(this.game, BedwarsRel._l(player, "ingame.specials.arrow-blocker.left",
    ...
    player
        .sendMessage(ChatWriter.pluginMessage(this.game,
  ...
  public Material getItemMaterial() {
    return Utils.getMaterialByConfig(this.game, "specials.arrow-blocker.item", Material.EYE_OF_ENDER);
  }
    ...
    this.task = new BukkitRunnable() {
      ...
      public void run() {
        ...
        if (protectionTime > 0 && ArrowBlocker.this.livingTime == protectionTime) {
          player.sendMessage(
              ChatWriter.pluginMessage(ArrowBlocker.this.game,
                  BedwarsRel._l(player, "ingame.specials.arrow-blocker.end")));
          isActive = false;
        }
